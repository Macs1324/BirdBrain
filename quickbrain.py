...
class Genetic_3L:
	...
	def neat(self, q_set, a_set, generations):
		for epoch in range(generations):
			...
			self.best_bot.feed_forward(curr_q, qm.sigmoid).printValue("guess of the best bot:")
...
def neat(n_i, n_h1, n_h2, n_h3, n_o, q_set, a_set, epochs, gen_size, cross_mode, activation, mutation_rate = 0.01):
	for epoch in range(epochs):
...
class Custom_DFF:
	def __init__(self, layers, activation=qm.sigmoid, cost=qm.Losses.MSE):
		...
		for w in range(len(self.weights)):
			self.weights[w] = qm.random_matrix(self.layers[w + 1].rows, self.layers[w].rows)
	...
	class Join:
		def __init__(self, neurons, activation, prev_layers):
			...
			for layer in prev_layers:
				self.weights.append(qm.random_matrix(self.values.rows, layer.values.rows))
			self.activation = activation
			self.bias = qm.clear_matrix(neurons, 1)
			self.error = None
		...
		def get_next_error(self):
			for l in range(len(self.prev_layers)):
				self.prev_layers[l].error = self.weights[l].transpose().times(self.error)
...
class Models:
	class Sequential:
		...
		def backpropagate(self, x0, yhat, y):
			...
			deltas.append(qm.from_numpy(self.cost.derivative(qm.to_numpy(y), qm.to_numpy(yhat)) * self.layers[-1].values.with_applied(self.layers[-1].activation.derivative)))
...
def DFF_paridispari():
	...
	q_set = qm.Matrix(data=[
		[1,2,3,4,5,6]
	]).transpose()

	a_set = qm.Matrix(data=[
		[0, 1],
		[1, 0],
		[0, 1],
		[1, 0],
...
def DFF_xor():
	...
	xor_dataset = qm.Matrix(data=[
		[1, 0],
		[0, 1],
		[1, 1],
		[0, 0]
	])
	xor_answers = qm.Matrix(data=[
		[1,1,0,0]
	]).transpose()

	brain.train(xor_dataset, xor_answers, 10000, 1)

	brain.feed_forward(qm.Matrix(data=[[0, 1]]).transpose(), qm.sigmoid).printValue("answer to 0, 1")
	brain.feed_forward(qm.Matrix(data=[[1, 1]]).transpose(), qm.sigmoid).printValue("answer to 1, 1")
...
def perc_exam():
	...
	p.feed_forward(qm.Matrix(data=[[7], [3]])).printValue("Il voto che prendi con 7h di studio e 3h di sonno")
